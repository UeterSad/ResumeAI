<template>
  ...
  const normalizeJobStatus = (jobItem) => ({
    ...jobItem,
    status: jobItem.status === 1 || jobItem.status === true
  })
  ...
  const loadJobPage = async () => {
    ...
    try {
      ...
      if (data.success) {
        const list = data.data.list.map(normalizeJobStatus)
  ...
  const showAddDialog = () => {
    ...
    Object.assign(jobForm, createEmptyJobForm())
  ...
  const showEditDialog = (row) => {
    ...
    rowData.status = normalizeJobStatus(rowData).status
