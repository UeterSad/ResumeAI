<template>
  <div class="recommend-page">
    ...
    <div v-else-if="!loading && searched" class="empty-state">
      ...
      <h3>暂无匹配岗位</h3>
      <p>当前未命中合适岗位，先优化简历并明确学习方向，再次匹配会更精准</p>
      <div class="guidance-panel">
        <div class="guidance-item">
          <h4>优化建议</h4>
          <p>{{ noMatchResumeImprovement }}</p>
        </div>
        <div class="guidance-item">
          <h4>学习方向</h4>
          <p>{{ noMatchLearningDirection }}</p>
        </div>
      </div>
...
export default {
  ...
  computed: {
    noMatchResumeImprovement() {
      return this.noMatchGuidance?.resume_improvement || '建议补充项目成果和关键数据，突出与目标岗位直接相关的能力证据。';
    },
    noMatchLearningDirection() {
      return this.noMatchGuidance?.learning_direction || '建议围绕目标岗位高频技能做系统学习，并通过项目实践形成可展示作品。';
    }
  },
  created() {
  ...
  methods: {
    normalizeRecommendations(jobList) {
      return jobList.map((jobItem) => {
        const normalizedScore = Math.min(jobItem.matchScore, 100);
        return { ...jobItem, matchScore: normalizedScore };
      });
    },
    async loadRecommendations() {
      ...
      this.loading = true;
      this.errorMessage = '';
      this.noMatchGuidance = null;
      
      try {
        const response = await axios.post('/api/recommend/jobs', { userId });
        
        if (response.data) {
          if (response.data.jobs) {
            this.recommendedJobs = this.normalizeRecommendations(response.data.jobs);
            this.careerAdvice = response.data.career_advice;
            this.noMatchGuidance = response.data.no_match_guidance || null;
          } else if (Array.isArray(response.data)) {
            this.recommendedJobs = this.normalizeRecommendations(response.data);
            this.noMatchGuidance = null;
          } else {
